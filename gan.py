...
class GAN(object):
    def __init__(self, data, gen, num_steps, batch_size, minibatch, log_every, anim_path):
        self.data = data             # 原始 数据分布Class
        self.gen = gen               # 生成 数据分布Class
        self.num_steps = num_steps   # 指定步长
        self.batch_size = batch_size # 指定批次大小 默认为12
        self.minibatch = minibatch   # 是否需要进行 minibatch 默认为FALSE
        self.log_every = log_every   # 每log_every次记录log函数值 默认为 10次
        self.mlp_hidden_size = 4     # ？？？
        self.anim_path = anim_path   # 动画存放的地方
        self.anim_frames = []        # 制作动画所需的所有帧数~
